...
(function ($) {
    ...
    $.fn.rpmPagination = function (options) {
        ...
        if (version == 4) {
            ...
            rpmHideClass = 'd-none', rpmAnchorClass = '';
        }
        ...
        $(document).on('click', '.page-item > a', function (e) {
            ...
            if ($(this).parent().hasClass('disabled') || $(this).parent($this).length === 0) {
                ...
                if (settings.refresh === true) {
                    ...
                    if ($(this).parent().hasClass('pagination-previous')) {
                        [settings.currentPage, rpmPageNext] = preparePageItems(settings.currentPage, rpmPageNext, settings.limit, rpmCustomDomElem, 'pagination-previous');
                    } else if ($(this).parent().hasClass('pagination-next')) {
                        [settings.currentPage, rpmPageNext] = preparePageItems(settings.currentPage, rpmPageNext, settings.limit, rpmCustomDomElem, 'pagination-next');
                    } else {
    ...
    function refreshPageforItems(obj, current, limit, url, form) {
        ...
        if (obj.parent().hasClass('pagination-previous')) {
            offset = limit * (current - 2);
        } else if (obj.parent().hasClass('pagination-next')) {
    ...
    var preparePageMenus = function (current_page, pages, element) {
		...
		//menuHtml = '<li class="pagination-previous page-item ' + (current_page === fp ? 'disabled' : '') + '"><a class="" href="#">Prev</a></li>';
		if(current_page === fp){
			menuHtml = '<li class="pagination-previous page-item disabled">Prev</li>';
		} else {
			menuHtml = '<li class="pagination-previous page-item"><a class="" href="#">Prev</a></li>';
		}
        ...
        for (let i = 0; i < pageArray.length; i++) {
            if (lp <= i) {
                ...
                if (pageArray[i] === '...') {
                    menuHtml += '<li class="ellipsis disabled page-item"><a class="" data-page_no="..." href="#"></a></li>';
                } else {
                    menuHtml += '<li class="page-item page-' + pageArray[i] + ' ' + (pageArray[i] === current_page ? 'current' : '') + '"><a class="" data-page_no="' + pageArray[i] + '" href="#">' + pageArray[i] + '</a></li>';
                }
            }
        }
        //menuHtml += '<li class="' + rpmListClass + ' next ' + (current_page === lp ? 'disabled' : '') + '"><a class="' + rpmAnchorClass + '" href="#">next</a></li>';
		//menuHtml += '<li class="pagination-next page-item ' + (current_page === lp ? 'disabled' : '') + '"><a class="" href="#">Next</a></li>';
        if(current_page === lp){
			menuHtml += '<li class="pagination-next page-item disabled">Next</li>';
		} else {
			menuHtml += '<li class="pagination-next page-item"><a class="" href="#">Next</a></li>';
		}
